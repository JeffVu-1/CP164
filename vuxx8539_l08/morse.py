...
def fill_letter_bst(bst, values):
    ...
    for i in range(len(values)):
        new_values.append(ByLetter(values[i][0], values[i][1]))

    for i in range(len(new_values)):
        bst.insert(new_values[i])
...
def fill_code_bst(bst, values):
    ...
    for i in range(len(values)):
        new_values.append(ByCode(values[i][0], values[i][1]))
...
def encode_morse(bst, text):
    ...
    for value in text:
        if value == ' ':
            ...
            current = ByLetter(value, None)
            retrive = bst.retrieve(current)
